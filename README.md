AdaptiPolish-Sys
AdaptiPolish-Sys 是一个基于点云与深度学习的缺陷检测自动初磨系统，通过融合机械臂控制、计算机视觉和实时监控技术，实现自动化磨抛作业。系统支持工件轮廓识别、路径规划、打磨参数配置等功能，适用于3D打印件、木材等材料的表面处理解决方案。
一、安装教程
1. 一键安装
...
二、配置机械臂连接
通过串口或网口将MyCobot 280连接到电脑
三、配置相机服务
通过网口连接到相机
打开CameraService.py，修改python_path的值为实际Python 3.9的路径
启动CameraService服务
四、点云C++部署
1. 搭建配置文件夹架构
例：
...
3. 编译C++点云库
创建一个pc_backend.cpp文件（具体参考资料）
创建一个CMakeLists.txt文件（具体参考资料）
在命令行中输入以下命令：
cd ~/Desktop/cse/pointcloud_project/src  # 请替换为您的自定义路径
rm -rf build
mkdir build
cd build
cmake ..
make -j4
完成后会自动生成一个.so文件，在Python中调用即可使用C++点云库。
五、启动系统
python main.py
...
二、核心功能操作指南
1. 机器人连接
在右侧“机器人连接”区域输入IP地址（默认：192.168.25.181）和端口（默认：9000）
点击“连接机器人”按钮
连接成功后状态指示灯变为绿色
...
在右侧“摄像头设置”区域：
选择摄像头类型（本地/网络）
输入网络摄像头的IP和端口（如使用网络摄像头）
加载ONNX模型文件（点击“加载”按钮）
点击“开始标定”按钮
将两个ArUco标记放置在相机视野内（相距220mm）
系统自动完成标定后显示“标定成功”
点击“获取点云数据”按钮，系统会自动调用3D相机扫描点云，与标准工件进行对比检测缺陷，返回缺陷点坐标信息
3D相机标定矩阵已集成在系统内部，返回缺陷点的坐标会自动转换
3. 示教点管理
将机械臂移动到目标位置
在中间“示教点管理”区域：
输入示教点名称
